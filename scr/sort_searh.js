function getArrayFromStorage() {
    ...
    for (let i = 0; i < keyNumbers; i++) {
        ...
        elm.id = keyName;
        elm.name = row.name;
        elm.channels = row.channels;
        elm.power = row.power;
        elm.connection = row.connection;
        elm.color = row.color;
...
function sortElements(){
    ...
    if (checkBox.checked == true){
       let sortArr = getArrayFromStorage()
        ...
        //Сортуємо масив об'єктів по полю power
        sortArr.sort(byField('power'));
...
function searchElements(){
  ...
  for (let i=0; i<searchtArr.length; i++) {
      ...
      let strC = row.channels;
      let strV = row.power;
      let strO = row.connection.toLowerCase();
      let strL = row.color;
      if (regExp.test(strN) || regExp.test(strC) || regExp.test(strV) || regExp.test(strO) || regExp.test(strL)) {buildElementToPage(row.id, row)
                                                                        isFounded = true}
      }
